...
femData <- read.csv(file = here("data/LiverFemale3600.csv"))
...
```{r}
traitData <- read.csv(file = here("data/ClinicalTraits.csv"))
dim(traitData)
names(traitData)
...
```{r}
# Re-cluster samples
sampleTree2 <- hclust(d = dist(datExpr),
                      method = "average")

# Convert traits to a color representation: white means low, red means high, grey means missing entry
traitColors <- numbers2colors(x = datTraits,
                              signed = FALSE)
...
```{r}
save(datExpr, datTraits, file = here("data/FemaleLiver-01-dataInput.RData"))
```