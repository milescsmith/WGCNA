...
stratifiedBarplot <- function(expAll, groups, split, subset,
  ...
  if (length(subset) > 1) {
    if (scale == "A") expSubset <- t(apply(expSubset, 1, function(x) {
      return(x / mean(x))
    }))
    if (scale == "Z") expSubset <- t(apply(expSubset, 1, function(x) {
  ...
  if (graph) {
    ...
    barplot(exp,
      beside = TRUE, legend.text = TRUE, main = subsetName, las = las1,
  ...
  out <- list(
    splitGroupMeans = exp, splitGroupSDs = std,
