...
bicor <- function(x, y = NULL, robustX = TRUE, robustY = TRUE, use = "all.obs", maxPOutliers = 1, quick = 0,
  ...
  if (nNA > 0) {
    warning(paste(
      "Missing values generated in calculation of bicor.",
  ...
  if ((method == "pearson") && ((na.method == 1) || (na.method == 3))) {
    ...
    if (!is.null(weights.x)) {
      ...
      if (any(!is.finite(weights.x))) {
        if (verbose > 0) {
          warning(
            "cor: found non-finite weights. These will be removed (set to missing), ",
    ...
    if (!is.null(weights.y)) {
      ...
      if (is.null(dim(weights.y))) {
        if (length(weights.y) != nrow(y)) {
          stop(
            "When 'weights.y' are given, they must be a vector of length 'nrow(y)' or a matrix\n",
    ...
    if (nNA > 0) {
      warning(paste(
        "Missing values generated in calculation of cor.",
