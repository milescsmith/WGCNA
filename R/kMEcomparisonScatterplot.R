...
kMEcomparisonScatterplot <- function(datExpr1, datExpr2, colorh, inA = NULL, inB = NULL,
                                     ...
                                     pch = 19,
                                     fileName = if (plotAll) {
                                       paste("kME_correlations_between_", nameA, "_and_", nameB, "_all.pdf", sep = "")
                                     } else {
  ...
  if (is.null(dim(datExpr1))) {
    write("Error: datExpr1 must be a matrix", "")
    return(0)
  }
  if (is.null(datExpr2)) {
    datA <- datExpr1[inA, ]
    ...
    if ((is.null(dim(datA))) | (is.null(dim(datB)))) {
      write("Error: Check input for inA and inB.", "")
      return(0)
    }
  } else {
    if (is.null(dim(datExpr2))) {
      write("Error: datExpr2 must be a matrix", "")
      return(0)
    }
    datA <- datExpr1
    datB <- datExpr2
  ...
  if ((dim(datA)[2] != length(colorh)) | (dim(datB)[2] != length(colorh))) {
    write("Error: Both sets of input data and color vector must all have same length.", "")
    return(0)
  }
  ...
  if (plotAll) {
    ...
    for (i in 1:length(mods)) {
      plotCol <- mods[i]
      if (mods[i] == "white") plotCol <- "black"
      verboseScatterplot(kMEsA[these, i], kMEsB[these, i],
        main = mods[i],
