#' .table2.allLevels
#' 
#' @description Accuracy measures, modified from the WGCNA version. 
#' 
#' @details Helper function: contingency table of 2 variables that will also include 
...
#'
#' @examples
.table2.allLevels <- function(
  x,
  y,
  levels.x = sort(unique(x)),
  levels.y = sort(unique(y)),
  setNames = FALSE
  ) {
  nx <- length(levels.x)
  ny <- length(levels.y)
...
accuracyMeasures <- 
  ...
  if (type == "binary") {
    if (is.null(dim(predicted))) {
      ...
      if (length(levels) != 2) {
        stop(
          "'levels' must contain 2 entries (the possible values of the binary variables\n",
    ...
    if (is.na(neg)) {
      stop(spaste(
        "Cannot find the negative level ", negativeLevel,
    ...
    if (is.na(pos)) {
      stop(spaste(
        "Cannot find the positive level ", positiveLevel,
        " among the colnames of the contingency table.\n   Please check the input and try again."
      ))
    }
    ...
    if (sum(is.na(tab))) {
      warning(
        "Missing data should not be present in input.\n",
    ...
    out <- data.frame(
      Measure = c(
        "Error.Rate", "Accuracy", "Specificity", "Sensitivity", "NegativePredictiveValue",
