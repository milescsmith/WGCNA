...
coxRegressionResiduals <- function(time, event, datCovariates = NULL) {
  ...
  if (length(time) != length(event)) {
    stop("Error: The length of the vector event is unequal to the length of the time vector. In R language: length(time) != length(event)")
  }
  ...
  if (!is.null(datCovariates)) {
    ...
    coxmodel <- eval(parse(
      text = paste(
        "survival:::coxph(Surv(time, event) ~ . , data=datCovariates,",
        "na.action = na.exclude, model = TRUE)"
      )
    ))
  } # end of if
  datResiduals <- data.frame(
    martingale = residuals(coxmodel, type = "martingale"),
