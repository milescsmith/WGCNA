...
.plotDendrogram <- function(tree, labels = NULL,
  ...
  if (horiz) {
    plot(NA, NA,
      xlim = if (reverseDirection) range else rev(range), ylim = indexLim,
      axes = axes, yaxt = "none", frame = FALSE, type = "n", xlab = xlab, ylab = ylab, ...
    )
  } else {
    plot(NA, NA,
      ylim = if (reverseDirection) rev(range) else range, xlim = indexLim,
  ...
  for (m in 1:nMerge)
  {
    ...
    if (!is.null(labels)) {
      if (horiz) {
        if (o1 < 0) {
          text(h1, x1, spaste(labels[-o1], " "),
            adj = c(0, 0.5), srt = 0,
            cex = cex.labels, xpd = TRUE
          )
        }
        if (o2 < 0) {
          text(h2, x2, spaste(labels[-o2], " "),
            adj = c(0, 0.5), srt = 0,
            cex = cex.labels, xpd = TRUE
          )
        }
        ...
        if (o1 < 0) {
          text(x1, h1, spaste(labels[-o1], " "),
            adj = c(1, 0.5), srt = 90,
            cex = cex.labels, xpd = TRUE
          )
        }
        if (o2 < 0) {
          text(x2, h2, spaste(labels[-o2], " "),
            adj = c(1, 0.5), srt = 90,
            cex = cex.labels, xpd = TRUE
          )
        }
